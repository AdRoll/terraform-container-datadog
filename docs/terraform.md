...
| apm\_enabled | Set to true to add DD\_APM\_ENABLED | `bool` | `true` | no |
| apm\_non\_local\_traffic | Set to true to add DD\_APM\_NON\_LOCAL\_TRAFFIC | `bool` | `true` | no |
...
| container\_port | Useful if docker labels are undefined | `number` | `8080` | no |
...
| ecs\_fargate | Set to true to add ECS\_FARGATE | `bool` | `true` | no |
...
| process\_agent\_enabled | Set to true to add DD\_PROCESS\_AGENT\_ENABLED | `bool` | `true` | no |
